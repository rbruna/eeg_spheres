function headmodel = mymcs_headmodel ( mesh, sens, varargin )
...
%               single sphere fit
%   baseline  = number
%   eqradii   = if true (default) all (inner) spheres will have the same
%               radius.
%
...
if any ( strcmp ( varargin, 'eqradii' ) ) && find ( strcmp ( varargin, 'eqradii' ), 1, 'last' ) < numel ( varargin )
    eqradii = varargin { find ( strcmp ( varargin, 'eqradii' ), 1, 'last' ) + 1 };
end
...
for chan = 1: nchans
    ...
    if size ( sphereps, 1 ) > 10
        ...
        if eqradii
            ...
            % Gets the unitary vector from the center to the sensor.
            vector = ( chanpos - o );
...
if any ( dradii ( :, 2 ) < minscalp )
    ...
    radii ( :, 3 ) = max ( radii ( :, 2 ) + minscalp, radii ( :, 3 ) );
end
