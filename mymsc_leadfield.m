function leadfield = mymsc_leadfield ( dips, sens, headmodel, moveinside )

% Leadfield for a EEG concentric spheres model.
% 
% Use as:
%   leadfield = mymsc_leadfield ( dippos, senspos, headmodel )
%
% Where:
...
% Uses the method of images in the sources outside the sphere.
if moveinside
...
%     end
    ...
    outcors = ones ( size ( radii ) );
end

% Uses the method of images to move the dipoles inside the inner sphere.
...
% Corrects the leadfield amplitude of the dipoles outside the sphere.
...
% Sorts the spheres from smallest to largest.
[ ~, indx ]    = sort ( headmodel.r );
headmodel.cond = headmodel.cond ( indx );
headmodel.r    = headmodel.r    ( indx );

% If only one sphere creates the other three.
...
if ~isfield ( headmodel, 't' )
    headmodel.t    = mymcs_gamma ( headmodel );
end
